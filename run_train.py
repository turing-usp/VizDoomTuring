...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--game-config", default=None, help="Caminho para o arquivo .cfg do jogo (ex: framework/tag.cfg).")
...
def build_multi_model_cmd(args: argparse.Namespace) -> List[str]:
    ...
    cmd: List[str] = [
        sys.executable,
        "-m",
        "vizdm_comp.framework.distributed_train_multi",
        "--num-matches",
        str(args.num_matches),
        "--game-port",
        str(args.port),
    ...
    _maybe_add_shm_obs(cmd, bool(args.shm_obs))
    if args.game_config:
        cmd.extend(["--game-config", args.game_config])
